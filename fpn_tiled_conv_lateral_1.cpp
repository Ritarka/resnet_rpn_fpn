...
{
#define N_TILE_ROWS (int) (LATERAL_CONV_1_IH/FPN_OUT_BUF_ROWS)
#define N_TILE_COLS (int) (LATERAL_CONV_1_IW/FPN_OUT_BUF_COLS)

#pragma HLS INTERFACE m_axi depth=512*92*160 	port=input_feature_map   bundle=fm
#pragma HLS INTERFACE m_axi depth=256*512*1*1	port=layer_weights       bundle=wt
...
#pragma HLS array_partition variable=partial_out_fm_buf dim=1 complete
    ...
    for(int ti = 0; ti < N_TILE_ROWS; ti++)
    {
        ...
        for(int tj = 0; tj < N_TILE_COLS; tj++)
        {
            ...
            FILTER_SIZE:
            for(int b = 0; b < (LATERAL_CONV_1_OD/FPN_OUT_BUF_CH); b++) 
            {
                // Divide tile cube into appropriate tile slices
                TILE_CUBE:
                for(int d = 0; d < (LATERAL_CONV_1_ID/FPN_IN_BUF_CH); d++) 
                {
                    // Load input tile slice from DRAM
                    fpn_load_input_tile_block_from_DRAM_1x1 <LATERAL_CONV_1_ID,LATERAL_CONV_1_IH,LATERAL_CONV_1_IW,N_TILE_ROWS,N_TILE_COLS> 
							(conv_in_buf, 
                                                     	input_feature_map, 
                                                     	ti, 
                                                     	tj, 
                    ...
                    fpn_load_layer_params_from_DRAM_1x1 <LATERAL_CONV_1_OD,LATERAL_CONV_1_ID>
						(conv_wt_buf, 
                                                 conv_bias_buf, 
                                                 layer_weights, 
                                                 layer_bias, 
		    ...
		    fpn_save_partial_output_tile_block<LATERAL_CONV_1_ID>
				(partial_out_fm_buf, 
                                 conv_out_buf, 
                                 conv_bias_buf, 
                                 d);
                ...
                fpn_store_output_tile_to_DRAM<LATERAL_CONV_1_OD,LATERAL_CONV_1_OH,LATERAL_CONV_1_OW>
					(output_feature_map, 
                                          partial_out_fm_buf, 
                                          ti, 
                                          tj, 
